...
pub fn run() !void {
    var rom = try Rom.from_file("test-roms/08-misc instrs.gb", std.testing.allocator);
    defer rom.deinit();
    ...
    // https://github.com/wheremyfoodat/Gameboy-logs - Blargg8LYStubbed.zip
    const file = try std.fs.cwd().openFile("test-roms/08-misc instrs.txt", .{});
    defer file.close();
...
// test "blargg 08-misc instrs log comparison" {
//     try run();
// }
